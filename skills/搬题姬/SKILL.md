...
4. **评估题目本身的难度**（入门/普及/提高/省选）
5. 记录判定结果和具体知识点，供后续阶段使用
...
**⚠️ 关键区分**：
- **知识点等级**：题目涉及的知识点在 GESP 考纲中所属的级别
- **难度等级**：题目本身的难度，可能低于知识点等级

**示例**：
- 一道题涉及"数位 DP"（七级知识点），但只是简单应用 → 难度：普及/提高-
- 一道题涉及"递归"（五级知识点），但只是基础模板 → 难度：入门
- 一道题涉及"枚举"（三级知识点），但需要复杂优化 → 难度：普及/提高-
...
#### 知识点与难度
本题涉及的知识点从属于 **GESP三级**（进制转换、位运算），难度等级：**入门**。

</div>
```

**⚠️ 必须添加知识点与难度说明**：在数据范围之后添加知识点与难度说明章节，格式为：
...
- **难度等级**：题目本身的难度（入门/普及-/普及/提高-/提高/省选-/省选/NOI）
- 两者可能不同：知识点可能是高级（如GESP七级），但题目难度可能较低（如普及）
- **必须同时标注两个维度，让用户清楚"学什么"和"难在哪"**
...
**⚠️ 重要**：`tag` 中必须添加 `GESPX级` 标签（知识点等级，使用阶段3判定的结果）
...
**注意**：题面中同时标注知识点等级和难度等级，problem.yaml 只标注知识点等级。
...
**变更记录**：
- 2026-04-08: 区分"知识点等级"和"难度等级"，题面和配置文件同时标注两个维度
- 2026-03-31: 初始版本
